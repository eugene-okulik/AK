...
def test_check_registration_form(driver):
    ...
    birthday_date = driver.find_element(By.ID, 'dateOfBirthInput')
    driver.execute_script("arguments[0].scrollIntoView();", birthday_date)
    birthday_date.click()
    ...
    user_subject.send_keys('Eng')
    wait = WebDriverWait(driver, 5)
    subject_values = wait.until(
        EC.visibility_of_element_located(
            (By.XPATH, '//div[contains(@id, "react-select") and text() = "English"]')
        )
    )
    subject_values.click()
    user_subject.send_keys('Phy')
    subject_values_2 = wait.until(
        EC.visibility_of_element_located(
            (By.XPATH, '//div[contains(@id, "react-select") and text() = "Physics"]')
        )
    )
    subject_values_2.click()
